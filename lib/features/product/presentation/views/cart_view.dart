          ...
          builder: (context, state) {
            ...
            return Column(
              children: [
                  ...
                  children: [
                    Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        boxShadow: [
                          BoxShadow(blurRadius: 14, spreadRadius: -8, offset: Offset(0, 6), color: AppColors.secondary.withAlpha(63)),
                          BoxShadow(blurRadius: 9, spreadRadius: -7, offset: Offset(0, -4), color: AppColors.secondary.withAlpha(63)),
                        ],
                      ),
                      child: Column(
                        ...
                        children: [
                          ...
                          SizedBox(height: 8.h),
                          Text(
                            cubit.selectedAddress ?? context.tr.pickAddress, // العنوان المختار أو النص الافتراضي
                            maxLines: 1,
                            overflow: TextOverflow.ellipsis,
                            style: AppTextStyles.textStyle12.copyWith(fontWeight: FontWeight.w400),
                          ),
                        ],
                      ),
                    ),
                    ...
                    GestureDetector(
                      onTap: () async {
                        final result = await context.push(AppRouterKeys.map); 
                        if (result != null && result is Map<String, dynamic>) {
                           cubit.updateAddress(result['address']);
                        }
                      },
                      child: Container(
                        width: 78.w, height: 78.h,
                        decoration: BoxDecoration(
                          borderRadius: BorderRadius.circular(6.r),
                        ...
                        child: Center(
                          child: SvgPicture.asset(AppSvgs.location, width: 33.w, height: 41.25.h, colorFilter: ColorFilter.mode(AppColors.white, BlendMode.srcIn)),
                        ),
                      ),
                    ),
                  ],
                ...
                // هنا بدأنا عرض محتوى الكارت (الـ List والـ Summary)
                Expanded(
                  child: cubit.cartItems.isEmpty
                      ? Center(
                          child: Column(
                            ...
                            children: [
                              Icon(Icons.shopping_cart_outlined, size: 80.r, color: AppColors.borderColor),
                              SizedBox(height: 16.h),
                              Text(context.tr.yourCartIsEmpty, style: AppTextStyles.textStyle18.copyWith(color: AppColors.borderColor)),
                            ],
                          ),
                        )
                      : Column(
                          ...
                          children: [
                            Padding(
                              ...
                              child: Text(context.tr.shoppingList, style: AppTextStyles.textStyle13.copyWith(fontWeight: FontWeight.bold, color: AppColors.secondary)),
                            ),
                            Expanded(
                              child: ListView.separated(
                                padding: EdgeInsets.zero,
                                itemCount: cubit.cartItems.length,
                                separatorBuilder: (context, index) => Divider(color: AppColors.lightgrey, height: 24.h),
                                itemBuilder: (context, index) => CartItemWidget(item: cubit.cartItems[index]),
                              ),
                            ),
                            ...
                            BlocConsumer<PlaceOrderCubit, PlaceOrderState>(
                              listener: (context, state) {
                                if (state is PlaceOrderSuccess) {
                                  AppNotifications.showSuccess(context, state.placeOrder.message ?? context.tr.orderPlacedSuccessfully);
                                  cubit.clearCart();
                              ...
                              builder: (context, state) {
                                ...
                                return state is PlaceOrderLoading
                                    ...
                                    : CustomButton(
                                        onPressed: () => orderCubit.placeOrder(items: cubit.cartItems),
